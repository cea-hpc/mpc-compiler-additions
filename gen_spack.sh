...
sed  "s/@VARIANTS_LIST@/$VARIANTS_LIST/g" < "${SCRIPTPATH}/config/spack_recipe.py" | sed "s/@GCC_VERSIONS_LIST@/$GCC_VERSIONS_LIST/g" | sed "s/@SPACK_VARIANT_FLAGS@/$SPACK_VARIANT_FLAGS/g" | sed "s/@SPACK_VARIANT_CONF_FLAGS@/$SPACK_VARIANT_CONF_FLAGS/g" | sed "s/@GCC_VERSION_CONFLICTS@/$GCC_VERSION_CONFLICTS/g" | sed "s/@AUTOPRIV_VERSION@/$AUTOPRIV_VERSION/g" | sed "s/@SHA256SUM@/$SHA256SUM/g" | tee ${PKG_OUTPUT}